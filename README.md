...
class Plot_RTS(Plot_KF):
    ...
    def plotTraj_CA(self, test_target, RTS_out, rtsnet_out, dim, file_name,
                    kf_sigma=None, n_sigma=3, sample=0):
        # [AJOUT COVARIANCE] kf_sigma optionnel : [N, m, m, T]. Si fourni, on
        # trace le couloir +/- n_sigma autour de l'estimee KF (RTS_out).
        legend = ["KalmanNet", "Ground Truth", "KF"]
        ylabels = {0: 'position', 1: 'velocity', 2: 'acceleration'}
        if dim not in ylabels:
            print("invalid dimension")
            return
        font_size = 14
        T_test = rtsnet_out[sample].size()[1]
        x_plt = range(0, T_test)

        kf_line = RTS_out[sample][dim, :]
        kf_np = kf_line.detach().cpu().numpy() if hasattr(kf_line, 'detach') else kf_line

        # couloir de covariance autour de l'estimee KF
        if kf_sigma is not None:
            var = kf_sigma[sample, dim, dim, :]
            var = var.detach().cpu().numpy() if hasattr(var, 'detach') else var
            std = np.sqrt(np.clip(var, 0, None))
            plt.fill_between(x_plt, kf_np - n_sigma*std, kf_np + n_sigma*std,
                             color='tab:blue', alpha=0.18,
                             label=rf'KF $\pm{n_sigma}\sigma$')

        plt.plot(x_plt, rtsnet_out[sample][dim, :].detach().cpu().numpy(), label=legend[0])
        plt.plot(x_plt, test_target[sample][dim, :].detach().cpu().numpy(), label=legend[1])
        plt.plot(x_plt, kf_np, label=legend[2])
        plt.legend(fontsize=font_size)
        plt.xlabel('t', fontsize=font_size)
        plt.ylabel(ylabels[dim], fontsize=font_size)
        plt.grid(alpha=0.3)
