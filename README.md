...
class SwarmEKF:
    def __init__(self, start_y2=10.0):
        ...
        # État estimé (L'EKF NE CONNAÎT PAS LES BIAIS)
...
plt.plot(time, dist_est, 'g-', linewidth=2, label="Distance Estimée (EKF)")
plt.title(f"Distance relative D1 <-> D2 : {état_ranging}")
plt.xlabel("Temps (s)")
...
for j in range(5):
    ...
    # Drone 2 (Rouge - Biaisé)
    plt.plot(time, X_t[:, j+5], 'k--', alpha=0.5)
    plt.plot(time, X_e[:, j+5], 'r-', label="D2 Estimé (BIAISÉ)")
    s2 = np.sqrt(P_e[:, j+5])
